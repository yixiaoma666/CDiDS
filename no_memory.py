    ...
    for _psi in [2, 4, 8, 16]:
        ...
        for _ in range(10):
            test_data = np.loadtxt(
                "my_data/data14.csv", delimiter=",")[:, :2]
            ans = np.loadtxt("my_data/data14.csv", delimiter=",")[:, 2].reshape(-1)
            myx = ADwCDiDS(test_data, _psi, _t=500, _window_size=500,
                           _alpha=a, _beta=b, _k=0.01, _init_window=100)
            predict = myx.detect()
            # ans = list(1+ans) # 原数据0为正常，1为异常，F1正常显示
            ans = list(2-ans)  # 原数据0为正常，1为异常，ROC正常显示
            ...
            for tp, ta in [(predict_1, ans_1),(predict_2, ans_2),(predict_3, ans_3),(predict, ans)]:
                fpr, tpr, thresholds = roc_curve(ta, tp, pos_label=2)
                f1_scores = []
                for thresh in thresholds:
                    f1_scores.append(
                        f1_score(ta, [1 if m > thresh else 2 for m in tp]))
